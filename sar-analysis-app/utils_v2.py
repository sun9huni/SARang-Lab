...
Original file is located at
    https://colab.research.google.com/drive/1FnVFSdMdBXC_UUl77GB2JYfTEjLJ7djJ
"""
...
def train_qsar_model(_df):
    """
    주어진 데이터로 QSAR 모델(RandomForestRegressor)을 훈련하고 성능을 평가합니다.
    훈련된 모델과 성능 지표(R2 Score)를 반환합니다.
    """
    df = _df.copy()
    df.dropna(subset=['SMILES', 'activity'], inplace=True)
    df['SMILES'] = df['SMILES'].astype(str)

    df['mol'] = df['SMILES'].apply(Chem.MolFromSmiles)
    df.dropna(subset=['mol'], inplace=True)

    if len(df) < 10: # 훈련에 필요한 최소 데이터 수
        return None, "훈련에 필요한 데이터가 부족합니다 (최소 10개).", None
    ...
    return model, f"모델 훈련 완료 (R² Score: {r2:.3f})", r2
